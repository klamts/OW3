...
import type {
  ...
  Unit4ActivitySection,
  Unit4VocabSection,
} from "../../../types";

export const SensesVocabulary1Tab: React.FC<{ data: Unit4Vocabulary1Data | null }> = ({ data }) => {
  ...
  // ✅ Tìm các section đúng thứ tự
  const vocabSection = data.sections.find(
    (s) => s.section_name === "Listen and Read"
  ) as Unit4VocabSection | undefined;
  console.log(vocabSection)
  const describeGuessData = data.sections.find(
    (s) => s.section_name === "describe guess"
  ) as Unit4ActivitySection | undefined;
  console.log(describeGuessData)
  const describeGuessItems = describeGuessData?.items || [];

  if ((!vocabSection?.words || vocabSection.words.length === 0) && describeGuessItems.length === 0) {
    return (
  ...
  return (
    <div className="space-y-12">
      ...
      {/* Vocabulary Section */}
      {vocabSection?.words && vocabSection.words.length > 0 && (
        <section>
          <h2 className="text-3xl font-bold mb-6 text-slate-800 dark:text-slate-200">Vocabulary</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {vocabSection.words.map((word, i) => (
              <VehicleCard
                key={i}
                vehicle={{
                  id: i.toString(),
                ...
                isActive={activeCardId === i.toString()}
                onClick={() => setActiveCardId(i.toString())}
      ...
      {describeGuessItems.length > 0 && currentItem && (
        <section>
          <h2 className="text-3xl font-bold mb-6 text-slate-800 dark:text-slate-200">
            {describeGuessData?.instruction || "Describe. Listen and guess. Work with a partner."}
          </h2>
            ...
            <div className="flex flex-col md:flex-row gap-6 items-center">
              <div className="w-full md:w-1/2 lg:w-1/3">
                <VehicleCard
                  ...
                  isActive={activeCardId === currentItem.word}
                  onClick={() => setActiveCardId(currentItem.word)}
                  isTextVisible={isTextVisible}
                />
              </div>
              ...
              <div className="w-full md:w-1/2 lg:w-2/3 p-4 bg-white dark:bg-slate-800 rounded-lg shadow-inner">
                ...
                <div className="flex items-start gap-2">
                  <div className="flex-grow">
                    {isTextVisible ? (
                      <p className="text-lg italic text-slate-800 dark:text-slate-200">
                        "{currentItem.example.text}"
                      </p>
                    ) : (
                      <div className="h-12 flex items-center text-slate-400 dark:text-slate-500 italic">
                        Example hidden
                      </div>
                    )}
                  </div>
                  <AudioButton src={currentItem.example.audio} title="Listen to example" />
                </div>
              </div>
            </div>
          </div>
