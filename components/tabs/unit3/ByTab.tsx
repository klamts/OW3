...
export const ByTab: React.FC<ByTabProps> = ({ byPhrases }) => {
  const [activeLineId, setActiveLineId] = useState<string | null>(null);
  const [currentByIndex, setCurrentByIndex] = useState<number>(0);
  const [isTextVisible, setIsTextVisible] = useState(true);

  if (byPhrases.length === 0) {
    return <div className="text-center p-10 text-slate-600 dark:text-slate-400">No content to display.</div>;
  }

  const totalLines = byPhrases.length;
  const currentLine = byPhrases[currentByIndex];

  const handlePrev = () => {
    if (currentByIndex > 0) {
      setCurrentByIndex(currentByIndex - 1);
    }
  };
  const handleNext = () => {
    if (currentByIndex < totalLines - 1) {
      setCurrentByIndex(currentByIndex + 1);
    }
  };
        ...
        {currentLine ? (
            <div className="p-4 bg-slate-200/50 dark:bg-slate-800/50 rounded-lg">
                <div className="flex items-center justify-between mb-4 pb-3 border-b border-slate-300 dark:border-slate-700">
                    <NavButton onClick={handlePrev} disabled={currentByIndex === 0}>
                        <ChevronLeftIcon className="w-6 h-6" />
                    </NavButton>
                    <div className="text-center">
                        <p className="text-sm text-slate-600 dark:text-slate-400">
                            {currentByIndex + 1} of {totalLines}
                        </p>
                    </div>
                    <NavButton onClick={handleNext} disabled={currentByIndex === totalLines - 1}>
