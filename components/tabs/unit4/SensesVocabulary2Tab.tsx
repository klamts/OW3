...
const Exercise1: React.FC<{ data: SensesVocab2Exercise1; isTextVisible: boolean }> = ({ data, isTextVisible }) => {
                        ...
                        return (
                            <div key={`ex1-q-${index}`} className="p-4 border border-slate-200 dark:border-slate-700 rounded-lg">
                                <div className="flex items-center gap-2 text-lg flex-wrap">
                                    ...
                                    {isTextVisible && <p className="text-slate-800 dark:text-slate-200">{parts[0]}</p>}
                                    <input
                                      type="text"
                                    ...
                                    />
                                    {isTextVisible && parts[1] && <p className="text-slate-800 dark:text-slate-200">{parts[1]}</p>}
                                </div>
...
const Exercise2: React.FC<{ data: SensesVocabulary2['exercise2']; isTextVisible: boolean }> = ({ data, isTextVisible }) => {
    return (
        <section>
            <h2 className="text-2xl font-bold mb-4 text-slate-800 dark:text-slate-200 border-b-2 border-slate-200 dark:border-slate-700 pb-2">
                {data.title}
                    ...
                    {data.groups.map(group => (
                        <div key={group.number} className="p-4 border border-slate-200 dark:border-slate-700 rounded-lg space-y-3">
                            <div className="flex items-center gap-3">
                                ...
                                <img src={group.imageSrc||group.image} alt={`Exercise image ${group.number}`} className="w-16 h-16 object-cover rounded-md shadow-sm" />
                            </div>
...
export const SensesVocabulary2Tab: React.FC<SensesVocabulary2TabProps> = ({ data }) => {
  ...
  return (
    <div className="space-y-12">
      ...
      {/* Exercise 1 */}
      {data.exercise1 && <Exercise1 data={data.exercise1} isTextVisible={isTextVisible} />}

      {/* Exercise 2 */}
      {data.exercise2 && <Exercise2 data={data.exercise2} isTextVisible={isTextVisible} />}
    </div>
  );
};
