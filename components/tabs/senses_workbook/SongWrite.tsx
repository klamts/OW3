        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    {data.words.map(w => <div key={w.word} className="text-center p-1 rounded-md bg-slate-100 dark:bg-slate-700/50"><img src={w.imageSrc||w.image} alt={w.word} className="w-full h-16 object-contain"/><p className="text-xs mt-1 h-4">{isTextVisible && w.word}</p></div>)}
                </div>
            </div>
             <div>
