...
import type { Vehicle, SensesDescribeGuess } from '../../types';

interface SensesVocabulary1TabProps {
  vocabulary: Vehicle[];
  describeGuessData: SensesDescribeGuess | null;
}

export const SensesVocabulary1Tab: React.FC<SensesVocabulary1TabProps> = ({ vocabulary, describeGuessData }) => {
  ...
  return (
    <div className="space-y-12">
      ...
      {describeGuessData && describeGuessItems.length > 0 && currentItem && (
        <section>
            <h2 className="text-3xl font-bold mb-6 text-slate-800 dark:text-slate-200 border-b-2 border-slate-200 dark:border-slate-700 pb-2">
                {describeGuessData.instructions}
            </h2>
