...
import type { 
  ...
  Unit1Vocabulary1VocabSection, 
  Unit1Vocabulary1SpeakingSection, 
  Vehicle, 
  DialogueLine 
} from '../../../types';
...
const VocabularySection: React.FC<{
  section: Unit1Vocabulary1VocabSection;
  ...
  return (
    <section>
      ...
      <p className="text-sm italic text-slate-500 dark:text-slate-400 mb-6">
        {isTextVisible ? section.instruction_vi : section.instruction}
      </p>
...
const SpeakingSection: React.FC<{
  section: Unit1Vocabulary1SpeakingSection;
  isTextVisible: boolean;
}> = ({ section, isTextVisible }) => {
  ...
  return (
    <section>
      ...
      <ActivityCard
        title=""
        instruction={isTextVisible ? section.instruction_vi : section.instruction}
        isTextVisible={true}
      >
        <div className="flex items-center justify-between mb-4 pb-3 border-b border-slate-300 dark:border-slate-700">
          ...
          <button
            onClick={() => setCurrentIndex((i) => Math.min(section.examples.length - 1, i + 1))}
...
// Main Tab Component
export const Unit1Vocabulary1Tab: React.FC<{
  data: Unit1Vocabulary1Data | null;
}> = ({ data }) => {
        ...
        switch (section.type) {
          case 'Vocabulary':
            return (
              <VocabularySection
                key={index}
                section={section as Unit1Vocabulary1VocabSection}
                isTextVisible={isTextVisible}
              />
            );
          case 'Speaking':
            return (
              <SpeakingSection
                key={index}
                section={section as Unit1Vocabulary1SpeakingSection}
                isTextVisible={isTextVisible}
              />
            );
          default:
