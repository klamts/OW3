        ...
        <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-6 border-b-2 border-slate-200 dark:border-slate-700 pb-2 gap-4">
            ...
            <div className="flex flex-wrap gap-2 self-start sm:self-center">
                 <button
                    ...
                    title={isTextVisible ? "Hide all text" : "Show all text"}
                >
                    {isTextVisible ? <EyeOffIcon className="w-5 h-5" /> : <EyeIcon className="w-5 h-5" />}
                    <span>{isTextVisible ? 'Hide Text' : 'Show Text'}</span>
                </button>
                <HideShowButton 
        ...
        <div className="p-4 bg-slate-200/50 dark:bg-slate-800/50 rounded-lg">
          ...
          {currentStatement ? (
              <div key={`statement-group-${currentStatementIndex}`} className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {statementLine && <DialogueLine line={statementLine} isActive={activeLineId === statementLine.id} onClick={() => setActiveLineId(statementLine.id)} isTextVisible={isTextVisible} />}
                {agreeingLine ? (
                    <DialogueLine line={agreeingLine} isActive={activeLineId === agreeingLine.id} onClick={() => setActiveLineId(agreeingLine.id)} isTextVisible={isTextVisible} />
                ) : <div />}
                {notAgreeingLine ? (
                    <DialogueLine line={notAgreeingLine} isActive={activeLineId === notAgreeingLine.id} onClick={() => setActiveLineId(notAgreeingLine.id)} isTextVisible={isTextVisible} />
                ) : <div />}
              </div>
          ) : (
        ...
        {currentExample ? (
            <div className="p-4 bg-slate-200/50 dark:bg-slate-800/50 rounded-lg">
                ...
                {currentExample.sentences.map((sentence, sIndex) => {
                    ...
                    return <DialogueLine key={line.id} line={line} isActive={activeLineId === line.id} onClick={() => setActiveLineId(line.id)} isTextVisible={isTextVisible} />;
                })}
                </div>
            </div>
        ) : (
