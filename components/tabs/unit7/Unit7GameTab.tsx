...
import type { Unit7GameData, Unit7GameItem } from '../../../types';
import { ActivityCard, AudioButton, UserRecordingControls } from '../senses_workbook/shared';

const Modal: React.FC<{ item: Unit7GameItem; onClose: () => void }> = ({ item, onClose }) => {
...
export const Unit7GameTab: React.FC<{ data: Unit7GameData | null }> = ({ data }) => {
    const [selectedItem, setSelectedItem] = useState<Unit7GameItem | null>(null);
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className="absolute inset-0 bg-black/40 rounded-full flex items-center justify-center">
                                            <span className="text-white font-bold text-sm text-center leading-tight">{item.question_type.split(' ').slice(0, 3).join(' ')}...</span>
                                        </div>
                                    </div>
                                );
                            })}
