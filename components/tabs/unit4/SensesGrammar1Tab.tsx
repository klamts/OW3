...
const Exercise1: React.FC<{ data: SensesGrammar1Exercise1, isTextVisible: boolean }> = ({ data, isTextVisible }) => {
    console.log(data)
    ...
    return (
        <section>
            ...
            <div className="p-4 bg-white dark:bg-slate-800 rounded-xl shadow-lg">
                {isTextVisible && <p className="text-sm text-slate-600 dark:text-slate-400 italic mb-3">{data.instruction}</p>}
                    ...
                    {data.questions.map((q, index) => (
                        <div key={`q-${index}`} className="flex flex-col md:flex-row items-start gap-4 p-4 border border-slate-200 dark:border-slate-700 rounded-lg">
                            <div className="w-full md:w-1/3 flex-shrink-0">
                                {q.imageSrc||q.image && <img src={q.imageSrc||q.image} alt={`Question ${index + 1}`} className="w-full h-auto object-cover rounded-md shadow-sm" />}
                            </div>
                            <div className="w-full md:flex-grow">
                                <div className="flex items-baseline gap-2 text-lg">
                                    <span className="font-bold">{index + 1}.</span>
                                    {isTextVisible && <p className="text-slate-800 dark:text-slate-200">{q.q.split('...')[0]}</p>}
                                    <input
                                      type="text"
                                    ...
                                    {!checked[index] ? (
                                        ...
                                        <div className={`p-3 rounded-md transition-colors ${isCorrect(index) ? 'bg-emerald-100 dark:bg-emerald-900/30' : 'bg-red-100 dark:bg-red-900/30'}`}>
                                           ...
                                           <div className="flex items-center gap-2 mt-1">
                                               {isTextVisible && <p className="text-slate-700 dark:text-slate-300">
                                                    Correct answer: <span className="font-semibold" dangerouslySetInnerHTML={{ __html: q.highlight }} />
                                               </p>}
                                               <AudioButton src={q.audioSrc || q.audio} title="Listen to correct answer" />
                                           </div>
                                           {isTextVisible && <p className="text-sm text-slate-500 dark:text-slate-400 mt-2 italic">{q.explain}</p>}
                                           {isTextVisible && q.translation && (
...
const Exercise2: React.FC<{ data: SensesGrammar1Exercise2, isTextVisible: boolean }> = ({ data, isTextVisible }) => (
    <section>
        <h2 className="text-2xl font-bold mb-4 text-slate-800 dark:text-slate-200 border-b-2 border-slate-200 dark:border-slate-700 pb-2">
            {data.title}
        </h2>
        <div className="p-4 bg-white dark:bg-slate-800 rounded-xl shadow-lg space-y-4">
            {isTextVisible && <p className="text-sm text-slate-600 dark:text-slate-400 italic">{data.instruction}</p>}
            ...
            <div className="p-3 bg-slate-100 dark:bg-slate-700/50 rounded-md">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <AudioButton src={data.sample.audioSrc || data.sample.audio} />
                </div>
            </div>
            <div>
                ...
                <ul className="space-y-2">
                    {data.suggested_answers.map((ans, index) => (
                        <li key={`sa-${index}`} className="flex items-center gap-2 text-slate-700 dark:text-slate-300 text-sm">
                            ...
                            <AudioButton src={ans.audioSrc||ans.audio} />
                        </li>
                    ))}
                </ul>
            </div>
...
const Exercise3: React.FC<{ data: SensesGrammar1Exercise3, isTextVisible: boolean }> = ({ data, isTextVisible }) => (
     <section>
        <h2 className="text-2xl font-bold mb-4 text-slate-800 dark:text-slate-200 border-b-2 border-slate-200 dark:border-slate-700 pb-2">
            {data.title}
        </h2>
                ...
                <ul className="space-y-2">
                    {data.examples.map((ex, index) => (
                        <li key={`ex-${index}`} className="flex items-center gap-2 text-slate-700 dark:text-slate-300 text-sm">
                            ...
                            <AudioButton src={ex.audioSrc|| ex.audio} />
                        </li>
                    ))}
                </ul>
            </div>
...
export const SensesGrammar1Tab: React.FC<{data:SensesGrammar1 | null}> = ({ data }) => {
    console.log(data)
    ...
    <div className="space-y-12">
      ...
      <section className="p-4 sm:p-6 bg-white dark:bg-slate-800 rounded-xl shadow-lg">
        <div className="flex items-center justify-between pb-4 border-b border-slate-200 dark:border-slate-700">
            ...
            <AudioButton src={data.introduction.audioSrc || data.introduction.audio} title={`Play intro: ${data.introduction.title}`} />
        </div>
                    ...
                    <li key={`grammar-item-${index}`} className="flex items-center gap-4 p-3 bg-slate-100 dark:bg-slate-800/50 rounded-lg">
                        <div className="flex-grow">
                            {isTextVisible ? (
                                ...
                                <div className="min-h-[3rem] flex items-center text-slate-400 dark:text-slate-500 italic">Text is hidden.</div>
                            )}
                        </div>
                        <AudioButton src={item.audioSrc|| item.audio} title={`Play: ${item.text}`}/>
                    </li>
                ))}
             </ul>
        </div>
      ...
      <section className="p-4 bg-blue-100/50 dark:bg-blue-900/20 border-l-4 border-blue-500 rounded-r-lg">
        ...
        {isTextVisible ? (
            ...
            <div className="min-h-[4rem] flex items-center text-slate-400 dark:text-slate-500 italic">Rule is hidden.</div>
        )}
      </section>
      
      <div className="space-y-12">
       {data.exercise || data.exercise2 || data.exercise3 ? (
        <>
            {data.exercise && (
            <Exercise1 data={data.exercise} isTextVisible={isTextVisible} />
            )}
            {data.exercise2 && (
            <Exercise2 data={data.exercise2} isTextVisible={isTextVisible} />
            )}
            {data.exercise3 && (
            <Exercise3 data={data.exercise3} isTextVisible={isTextVisible} />
            )}
        </>
        ) : (
