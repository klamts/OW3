...
import type { Unit6Vocabulary3Data, Vehicle } from '../../../types';
import { VehicleCard } from '../../VehicleCard';
import { EyeIcon, EyeOffIcon } from '../../IconComponents';

export const Unit6Vocabulary3Tab: React.FC<{ data: Unit6Vocabulary3Data | null }> = ({ data }) => {
    const [activeCardId, setActiveCardId] = useState<string | null>(null);
    const [isTextVisible, setIsTextVisible] = useState(true);
    
    if (!data || !data.words || data.words.length === 0) {
        return <div className="text-center p-10 text-slate-600 dark:text-slate-400">No content for Unit 6 Vocabulary 3.</div>;
    }
