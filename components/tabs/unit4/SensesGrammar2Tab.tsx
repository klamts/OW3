    ...
    <div className="space-y-12">
      ...
      <section className="p-4 sm:p-6 bg-white dark:bg-slate-800 rounded-xl shadow-lg">
        ...
        <div className="mt-4 flex flex-col md:flex-row gap-6 items-center">
            {isTextVisible && <p className="text-slate-700 dark:text-slate-300 leading-relaxed md:w-1/2" dangerouslySetInnerHTML={{ __html: data.introduction.text.replace(/\*\*(.*?)\*\*/g, '<strong class="text-blue-600 dark:text-blue-400">$1</strong>') }} />}
            {data.introduction.imageSrc && <img src={data.introduction.imageSrc||data.introduction.image} alt="was/were chart" className="md:w-1/2 rounded-lg shadow-md" />}
        </div>
      </section>
              ...
              {data.items.map((item, index) => (
                  <div key={index} className="p-4 bg-white dark:bg-slate-800 rounded-xl shadow-lg">
                      <div className="flex items-center gap-4">
                          <img src={item.imageSrc||item.image} alt={item.word} className="w-20 h-20 object-cover rounded-lg shadow" />
                          <div>
                              <div className="flex items-center gap-2">
                                <h3 className="text-3xl font-bold text-emerald-600 dark:text-emerald-400">{item.word}</h3>
                                <AudioButton src={item.audioSrc||item.audio} />
                              </div>
                              {isTextVisible && <p className="text-slate-500 dark:text-slate-400 italic">{item.vi}</p>}
                          </div>
                      </div>
                      <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-700">
                          ...
                          <div className="flex items-center gap-4 p-3 bg-slate-100 dark:bg-slate-700/50 rounded-lg">
                              <img src={item.example.imageSrc||item.example.image} alt={item.example.text} className="w-16 h-16 object-cover rounded-md shadow"/>
                              <div className="flex-grow">
                                  {isTextVisible ? (
                                    ...
                                    <div className="h-12 flex items-center text-slate-400 dark:text-slate-500 italic">Example hidden</div>
                                  )}
                              </div>
                              <AudioButton src={item.example.audioSrc||item.example.audio} />
                          </div>
                      </div>
                  </div>
              ))}
      ...
      {data.exercise1 && (
        <section>
            ...
            <div className="p-4 bg-white dark:bg-slate-800 rounded-xl shadow-lg space-y-6">
                {isTextVisible && <p className="text-sm text-slate-600 dark:text-slate-400 italic">{data.exercise1.instruction}</p>}
                    ...
                    <div key={index} className="flex flex-col md:flex-row items-center gap-4 p-3 border border-slate-200 dark:border-slate-700 rounded-lg">
                        <div className="flex-1 space-y-2 text-center md:text-left">
                            ...
                            <div className="flex items-center gap-2 justify-center md:justify-start">
                                <img src={q.imageSrc||q.image} alt="Question" className="w-12 h-12 object-cover rounded-md"/>
                                {isTextVisible && <p className="text-slate-800 dark:text-slate-200">{q.q}</p>}
                                <AudioButton src={q.audioSrc||q.audio} />
                            </div>
                        </div>
                        ...
                        <div className="flex-1 space-y-2 text-center md:text-left">
                            ...
                            <div className="flex items-center gap-2 justify-center md:justify-start">
                                <img src={q.answer.imageSrc||q.answer.image} alt="Answer" className="w-12 h-12 object-cover rounded-md"/>
                                {isTextVisible && <p className="text-slate-800 dark:text-slate-200">{q.answer.text}</p>}
                                <AudioButton src={q.answer.audioSrc||q.answer.audio} />
                            </div>
                        </div>
                    </div>
                ))}
      ...
      {data.exercise2 && (
        <section>
            ...
            <div className="p-4 bg-white dark:bg-slate-800 rounded-xl shadow-lg space-y-4">
                {isTextVisible && <p className="text-sm text-slate-600 dark:text-slate-400 italic">{data.exercise2.instruction}</p>}
                {data.exercise2.questions.map((q, index) => (
                    <div key={index} className="flex flex-col sm:flex-row items-center gap-4 p-3 bg-slate-100 dark:bg-slate-700/50 rounded-lg">
                        <img src={q.imageSrc||q.image} alt="Sentence illustration" className="w-24 h-24 object-cover rounded-md shadow"/>
                        <div className="flex-grow">
                            {isTextVisible ? (
                                <>
                                ...
                                <div className="h-12 flex items-center text-slate-400 dark:text-slate-500 italic">Sentence hidden</div>
                            )}
                        </div>
                        <AudioButton src={q.audioSrc||q.audio} />
                    </div>
      ...
      {data.exercise3 && (
        <section>
            ...
            <div className="p-4 bg-white dark:bg-slate-800 rounded-xl shadow-lg space-y-4">
                {isTextVisible && <p className="text-sm text-slate-600 dark:text-slate-400 italic">{data.exercise3.instruction}</p>}
                 <div className="p-3 bg-amber-100 dark:bg-amber-900/30 rounded-lg border-l-4 border-amber-500">
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-4">
                        <img src={data.exercise3.sample.imageSrc||data.exercise3.sample.image} alt="Game sample" className="w-24 h-24 object-cover rounded-md shadow"/>
                        <div className="flex-grow">
                             {isTextVisible ? (
                                <>
                                ...
                                <div className="h-12 flex items-center text-slate-400 dark:text-slate-500 italic">Sample hidden</div>
                             )}
                        </div>
                        <AudioButton src={data.exercise3.sample.audioSrc||data.exercise3.sample.audio} />
                    </div>
                </div>
            </div>
        </section>
